...
def gen_chengjiao_insert_command(info_dict):
    ...
    生成成交记录数据库插入命令
    """
    info_list = [u'链接', u'小区名称', u'户型', u'面积', u'朝向', u'装修', u'电梯', u'楼层', u'建造时间', u'签约时间', u'签约单价', u'签约总价', u'房产类型',
                 u'学区', u'地铁']
    t = []
    for il in info_list:
        if il in info_dict:
...
def xiaoqu_spider(db_xq, url_page=u"http://gz.lianjia.com/xiaoqu/pg1rs%e5%a4%a9%e6%b2%b3/"):
    """
    爬取页面链接中的小区信息
    """
    try:
    ...
    for xq in xiaoqu_list:
        ...
        content = unicode(xq.find('div', {'class': 'positionInfo'}).renderContents().strip().replace("\n", ""))
        info = re.match(r".+>(.+)</a>.+>(.+)</a>.+/(.+)", content)
        if info:
            info = info.groups()
            info_dict.update({u'大区域': info[0]})
            info_dict.update({u'小区域': info[1]})
            info_dict.update({u'建造时间': info[2][:5].strip()})
        command = gen_xiaoqu_insert_command(info_dict)
        db_xq.execute(command, 1)


def do_xiaoqu_spider(db_xq, region=u"天河"):
    """
    爬取大区域中的所有小区信息
    """
...
def chengjiao_spider(db_cj, url_page=u"https://gz.lianjia.com/chengjiao/rs%E8%8A%B3%E8%8D%89%E5%9B%AD/"):
    """
    爬取页面链接中的成交记录
    """
    try:
    ...
    for cj in cj_list:
        ...
        content = cj.find('div', {'class': 'dealHouseInfo'}).findAll('span')
        if content:
            for c in content:
                c = c.text.strip()
                if c.find(u'满') != -1:
...
def xiaoqu_chengjiao_spider(db_cj, xq_name=u"芳草园"):
    """
    爬取小区成交记录
    """
...
if __name__ == "__main__":
    ...
    orientation TEXT, fitment TEXT, elevator TEXT, floor TEXT, year TEXT, sign_time TEXT, unit_price TEXT, \
